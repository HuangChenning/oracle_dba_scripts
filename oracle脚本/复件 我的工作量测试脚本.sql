select * from (
   select   substr(a.year_month,0,6) 年月,substr(a.year_month,8,8)  周 ,a.测试人员 ,
...
select   rq_req_name 需求名称,
         ...
         to_char(d.ts_creation_date,'yyyymm') year_month
...
select   rq_req_name 需求名称,
         ...
         to_char(e.RN_EXECUTION_DATE,'yyyymm') year_month
...
SELECT   rq_req_name 需求名称,
         ...
         to_char(BG_CLOSING_DATE,'yyyymm') year_month
...
select  rq_req_name 需求名称,
         ...
         RQ_USER_04 需求状态,
         RQ_USER_01 测试人员,
       to_char(to_date(RQ_USER_08, 'yyyy-mm-dd hh24:mi:ss'), 'yyyymm') year_month
...
select  rq_req_name 需求名称,
         ...
         RQ_USER_04 需求状态,
         BG_DETECTED_BY 测试人员,
         to_char(nvl(to_date(RQ_USER_08, 'yyyy-mm-dd hh24:mi:ss'),sysdate),'yyyymm') year_month
   from BOSS_BOSSTEST0723_DB.ALL_LISTS a,BOSS_BOSSTEST0723_DB.ALL_LISTS b,BOSS_BOSSTEST0723_DB.ALL_LISTS c, BOSS_BOSSTEST0723_DB.bug d,BOSS_BOSSTEST0723_DB.req e
  where a.AL_FATHER_ID=b.al_item_id
  and b.AL_FATHER_ID=c.al_item_id
  and d.BG_SUBJECT   =a.al_item_id
...
where d.TS_SUBJECT   =a.al_item_id
  ...
  and c.al_item_id=b.AL_FATHER_ID
  and rq_req_name =any(a.al_description,b.al_description,c.al_description)
  group by  RQ_REQ_NAME,to_char(d.ts_creation_date,'yyyymm'),d.ts_responsible
) b,
(
select   RQ_REQ_NAME 需求名称,
         count(*)         执行,
 ...
 where 
  ...
  and rq_req_name =any(a.al_description,b.al_description,c.al_description)
and e.RN_TEST_ID =d.ts_test_id
group by  RQ_REQ_NAME,to_char(e.RN_EXECUTION_DATE,'yyyymm'),e.RN_TESTER_NAME  
) c,
(
 select  RQ_REQ_NAME 需求名称,
       count(*)       缺陷,
       BG_DETECTED_BY 测试人员,
       to_char(d.BG_DETECTION_DATE,'yyyymm') year_month
 from BOSS_BOSSTEST0723_DB.ALL_LISTS a,BOSS_BOSSTEST0723_DB.ALL_LISTS b,BOSS_BOSSTEST0723_DB.ALL_LISTS c, BOSS_BOSSTEST0723_DB.bug d,BOSS_BOSSTEST0723_DB.req e
where a.AL_FATHER_ID=b.al_item_id
and b.AL_FATHER_ID=c.al_item_id
and d.BG_SUBJECT   =a.al_item_id
and rq_req_name =any(a.al_description,b.al_description,c.al_description)
group by  RQ_REQ_NAME,to_char(d.BG_DETECTION_DATE,'yyyymm'),BG_DETECTED_BY
) d,
(
  select  RQ_REQ_NAME 需求名称,
         count(*)             修复,
  ...
  and rq_req_name =any(a.al_description,b.al_description,c.al_description)
  and  BG_STATUS='已关闭'
  group by  RQ_REQ_NAME,to_char(BG_CLOSING_DATE,'yyyymm'),BG_DETECTED_BY
) e,
(
  select RQ_REQ_NAME 需求名称,
       1 需求关闭,
...
(
  ...
  and rq_req_name =any(a.al_description,b.al_description,c.al_description)
  and  BG_STATUS='遗留'
  group by  RQ_REQ_NAME,to_char(nvl(to_date(RQ_USER_08, 'yyyy-mm-dd hh24:mi:ss'),sysdate),'yyyymm'),BG_DETECTED_BY
) g
where a.需求名称=b.需求名称(+)
  and a.需求名称=c.需求名称(+)
  and a.需求名称=d.需求名称(+)
...
) where 测试人员 like'srd_huang%' order by 年月 desc